...
| 3 | **Component** | Scoped overrides — sizes for one component | `--card-title` → `var(--font-subheading)` |
...
Notice how each component defines its own font-size variables but still references the theme or scale tiers. The pricing card uses `--card-amount: var(--scale-2xl)` (scale) and `--card-desc: var(--font-secondary)` (theme). The sidebar nav references theme tokens for all its sizes. If the global type scale changes, these components update automatically.
